...
const char *path = "dataset/webdocs.dat";
const char *res_path_cmp = "result/result_docs/DOCS_THETA.csv";
const char *res_param_path = "result/result_docs/docs_para_FILTER.csv";
const char *res_path_cmp_pie = "result/result_docs/docs_pie_03.csv";
int TOPNUMBER = 10;
int how_many_items = 20000000;

vector<pair<uint64_t, int>> traces;
                     ...
                     int flow_thres, double portion_thres)
{
    ...
    readTraceWebDocs(path, traces);
    random_device rd;
    mt19937 rng(rd());

    // 打乱 vector 的顺序          取决于随机数的选择！！！  但绝对是打乱之后好用的！！！！
    shuffle(traces.begin(), traces.end(), rng);

    for (auto it1 = traces.begin(); it1 != traces.end(); ++it1)
    {
        it1->second = it1->second / 500 + 1;
    }
              ...
              double FRT, int NET, int NCE, int LAM, int THE, int MORS, int FTHRES)
{
    ...
    for (int i = 0; i < S1; i++)
    {
        basics[i] = new PIE_BAISIC<uint64_t>(MEM_RANGE[i], f_ratio, n_ce, n_et, lambda, theta, f_thres);
        hists[i] = new Hist_01<uint64_t>(MEM_RANGE[i], 16, 0.5); // n_slots & cm_ratio,默认配置
        cmccs[i] = new CMCC<uint64_t>(MEM_RANGE[i], 16, 3);
        mors[i] = new PIE_M1<uint64_t>(MEM_RANGE[i], f_ratio, n_ce, n_et, lambda, theta, f_thres, mor_size);
    }
    ...
    for (int i = 0; i < S1; i++)
    {
        ...
        resfile_cmp << MEM_RANGE[i] << "," << are_pies[i] / total_estimated_items << "," << are_mors[i] / total_estimated_items <<endl;
        cout << RED_TEXT << endl;
        printf("MEMORY = %d;\n", MEM_RANGE[i]);
        cout << BLUE_TEXT;
...
{
        // // //!  参数调节！
        // int param_mem_range[] = {300,400,500,600,700,800};
        // int param_filter_thresh_range[] = {25};
        // double param_filter_percent_range[] = {0.2};      //??? filter 相关调参图，就用webdocs
        // int param_theta_range[] = {4};  //*4
        // int param_lambda_range[] = {2}; //*2
        // int param_ncells_range[] = {8}; //*8
        // int param_nentries_range[] = {8}; //*8->16 !
        // int param_mor_size_range[] = {8};
        // int param_flow_thres = 1000;
    ...
    int cmp_flow_thres = 1000; 
    double cmp_portion_thres = 0.1;
    // Comp_Lat(cmp_mem_range, size(cmp_mem_range), cmp_flow_thres, cmp_portion_thres, f_ratio, n_et, n_ce, lambda, theta, mor_size, f_thres);
