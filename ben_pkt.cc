...
const char *path = "dataset/caida2016_pkt.txt";
const char *res_path_cmp = "result/result_pkt/PKT_CMP.csv";
const char *res_param_path = "result/result_pkt/PARAM_NET.csv";
const char *res_path_cmp_pie = "result/result_pkt/CMP_03.csv";
int TOPNUMBER = 10;
int how_many_items = 20000000;
                     ...
                     int flow_thres, double portion_thres)
{
                        ...
                        for (int x = 0; x < s3; ++x)
                        {
                            ...
                            for (int o = 0; o < s7; ++o)
                            {
                                for (int p = 0; p < s8; ++p)
                                {
                                    ...
                                    vf_pies.push_back(new PIE_BAISIC<uint64_t>(_mem, _filter_percent, _n_cells, _n_entries, _lambda, _theta, _filter_thres));
                                    vf_mors.push_back(new PIE_M1<uint64_t>(_mem, _filter_percent, _n_cells, _n_entries, _lambda, _theta, _filter_thres, _mor_size));
    ...
    int ss = how_many_items;
    cout << "TOTAL ITEMS =" << how_many_items << endl;
    ...
    for (int i = 0; i < how_many_sketches; i++)
    {
        ...
        double insert_tpr_pie = how_many_items / total_insert_times_pie[i];
        double insert_tpr_mor = how_many_items / total_insert_times_mor[i];
...
void Comp(int MEM_RANGE[], int S1, int FLOW_THRES_CMP, double PORTION_THRES_CMP,
          double FRT, int NET, int NCE, int LAM, int THE, int MORS, int FTHRES)
{
    ...
    readTrace(path, traces);
    cout << "TOTAL_DATA = " << traces.size() << endl;
    ...
    map<int, double> total_insert_times_simd;
    ...
    for (auto it = traces.begin(); it != traces.end(); it++)
    {
        ...
        {
            auto t_a = chrono::high_resolution_clock::now();
            basics[i]->insert(srcFp, pkt_size);
            auto t_b = std::chrono::high_resolution_clock::now();
            ...
            simds[i]->insert((uint16_t)srcFp, pkt_size);
            auto t_b_4 = std::chrono::high_resolution_clock::now();
            total_insert_times_simd[i] += chrono::duration_cast<std::chrono::microseconds>(t_b_4 - t_a_4).count();
        }
        ground_truth_all[src][pkt_size]++;
        ground_truth_size[src]++;
    }
    ...
    int total_estimated_flows = 0;
    int total_estimated_items = 0;
    map<int, double> are_simds;
    map<int, double> query_times_simds;
    ...
    for (auto it = sorted_ground_truth.begin(); it != sorted_ground_truth.end(); ++it)
    {
        ...
        for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2)
        {
            ...
            for (int i = 0; i < S1; i++)
            {
                ...
                auto t_a_5 = chrono::high_resolution_clock::now();
                simd_res = simds[i]->point_query((uint16_t)(it->first), it2->first);
                auto t_b_5 = std::chrono::high_resolution_clock::now();
                query_times_simds[i] += chrono::duration_cast<std::chrono::microseconds>(t_b_5 - t_a_5).count();
                are_simds[i] += abs((double)simd_res - (double)it2->second) / it2->second;
            }
        }
    }
    ...
    for (int i = 0; i < S1; i++)
    {
        resfile_cmp << MEM_RANGE[i] << "," << are_pies[i] / total_estimated_items << "," << are_mors[i] / total_estimated_items << "," << are_hists[i] / total_estimated_items << "," << are_cmccs[i] / total_estimated_items
                    << "," << how_many_items / total_insert_times_basic[i] << "," << how_many_items / total_insert_times_mor[i] << ","
                    << how_many_items / total_insert_times_hist[i] << "," << how_many_items / total_insert_times_cmcc[i] << "," << total_estimated_items / query_times_pies[i] << "," << total_estimated_items / query_times_mors[i]
        ...
        printf("PIE_INSERT_TPR = %.6fMOPS;\t", how_many_items / total_insert_times_basic[i]);
        printf("PIE_QUERY_TPR = %.6fMOPS;\n", total_estimated_items / query_times_pies[i]);

        // printf("MOR_ARE = %.6f;\t", are_mors[i] / total_estimated_items);
        // printf("MOR_INSERT_TPR = %.6fMOPS;\t", how_many_items / total_insert_times_mor[i]);
        // printf("MOR_QUERY_TPR = %.6fMOPS;\n", total_estimated_items / query_times_mors[i]);

        // printf("Hist_ARE = %.6f;\t", are_hists[i] / total_estimated_items);
        // printf("Hist_INSERT_TPR = %.6fMOPS;\t", how_many_items / total_insert_times_hist[i]);
        // printf("Hist_QUERY_TPR = %.6fMOPS;\n", total_estimated_items / query_times_hists[i]);

        // printf("CM_ARE = %.6f;\t", are_cmccs[i] / total_estimated_items);
        // printf("CM_INSERT_TPR = %.6fMOPS;\t", how_many_items / total_insert_times_cmcc[i]);
        // printf("CM_QUERY_TPR = %.6fMOPS;\n", total_estimated_items / query_times_cmccs[i]);
        // printf("\n");

        printf("SIMD_ARE = %.6f;\t", are_simds[i] / total_estimated_items);
        printf("SIMD_INSERT_TPR = %.6fMOPS;\t", how_many_items / total_insert_times_simd[i]);
        printf("SIMD_QUERY_TPR = %.6fMOPS;\n", total_estimated_items / query_times_simds[i]);
        printf("\n");
    }
}
              ...
              double FRT, int NET, int NCE, int LAM, int THE, int MORS, int FTHRES)
{
    ...
    for (auto it = sorted_ground_truth.begin(); it != sorted_ground_truth.end(); ++it)
    {
        ...
        for (int i = 0; i < S1; i++)
        {
            ...
            // auto t_a_2 = chrono::high_resolution_clock::now();
            auto t_a_2 = std::chrono::system_clock::now();
            pie_of_hist = hists[i]->pie_query(it->first);
            auto t_b_2 = std::chrono::system_clock::now();
            ...
            // double tp = calculateTotalDifference(it->first, truth_vector, pie_of_baisc, PORTION_THRES_PIE);
            // double tp2=calculateTotalDifference(it->first, truth_vector, pie_of_mor, PORTION_THRES_PIE);
            // cout<<tp<<" "<<tp2<<endl;
...
int main()
{
    ...
    // int param_theta_range[] = {2};
    // int param_lambda_range[] = {4};
    // int param_ncells_range[] = {16}; //* TESTING
    // int param_nentries_range[] = {4,8,12,16,20,32}; //4-8-12-16-20-32
    // int param_mor_size_range[] = {12};
    ...
    int lambda = 3;
    int mor_size = 12;
    Comp(cmp_mem_range, size(cmp_mem_range), cmp_flow_thres, cmp_portion_thres,
         f_ratio, n_et, n_ce, lambda, theta, mor_size, f_thres);

    // double f_ratio = 0.025;
    // int f_thres = 10;
    // int pie_cmp_mem_range[] = {300, 400, 500, 600, 700, 800};
    // int n_et = 6; // 4->8
    // int n_ce = 8;
    // int theta = 2;
    // int lambda = 3;
    // int mor_size = 12;
    // int pie_cmp_flow_thres = 700;
    // double pie_cmp_portion_thres = 0.1;
    // Comp_PIE(pie_cmp_mem_range, size(pie_cmp_mem_range), pie_cmp_flow_thres, pie_cmp_portion_thres, f_ratio, n_et, n_ce, lambda, theta, mor_size, f_thres);
    return 0;
}
